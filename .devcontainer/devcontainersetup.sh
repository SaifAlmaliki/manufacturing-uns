...
# 2.3 MQTT used by all modules (HiveMQ Edge — same image as Compose uns_mqtt_broker)
# trunk-ignore(shellcheck/SC2312)
if [[ -n $(docker ps -aq -f name=uns_emqx_mqtt) ]]; then
	docker rm -f uns_emqx_mqtt >/dev/null 2>&1 || true
fi
# trunk-ignore(shellcheck/SC2312)
if [[ -n $(docker ps -aq -f name=uns_mqtt_broker) ]]; then
	docker start uns_mqtt_broker
else
	docker run -d \
		--name uns_mqtt_broker \
		--restart unless-stopped \
		-p 1883:1883 \
		-p 18080:8080 \
		-v "${WORKSPACE_DEFAULT_PATH}/conf/hivemq/config.xml:/opt/hivemq/conf/config.xml:ro" \
