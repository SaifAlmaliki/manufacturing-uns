...
def test_apply_enterprise_to_settings_keeps_the_uns_wildcard():
    snippet = """\
graphdb:
  mqtt:
    topics: ["#"]
...
"""
    ...
    doc = yaml.safe_load(text)
    assert doc["graphdb"]["mqtt"]["topics"] == ["#"]
    assert doc["historian"]["mqtt"]["topics"] == ["#"]
    assert doc["kafka_mapper"]["mqtt"]["topics"] == ["#"]
