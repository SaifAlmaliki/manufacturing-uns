...
export const KafkaStreamsView: React.FC = () => {
  ...
  useEffect(() => {
    validateAndSubscribe(topicInput);
  }, []);
    ...
    <PageShell id="kafka-streams-view" scroll={false} className="flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 h-full overflow-hidden">
        ...
        <div className={`lg:col-span-7 flex flex-col h-full border-r border-zinc-800 bg-[#111114] overflow-hidden ${
          ...
          <div className="p-3 border-b border-zinc-800 space-y-2.5 shrink-0">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="font-bold text-[#0F172A] dark:text-[#F8FAFC] text-xs uppercase tracking-wider">Kafka Stream Monitor</h2>
                  <div className="flex items-center gap-2 text-[10px] text-[#64748B]">
                    <span>Throughput: <b className="text-emerald-600 dark:text-[#10B981]">{msgRate} msg/s</b></span>
                    <span>•</span>
                    <span>Buffer: <b className="text-[#0F172A] dark:text-[#F8FAFC]">{messages.length}/{bufferSize}</b></span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className={`flex items-center gap-1 px-2.5 py-1 rounded text-[10px] font-mono font-medium transition-colors cursor-pointer ${
                    isPaused
                      ? 'bg-amber-100 dark:bg-amber-600/20 border border-amber-300 dark:border-amber-500/40 text-amber-800 dark:text-amber-300'
                      : 'bg-white dark:bg-[#0B0B0C] border border-[#CBD5E1] dark:border-[#1E293B] hover:bg-slate-100 dark:hover:bg-[#1E293B] text-[#475569] dark:text-[#94A3B8]'
                  }`}
                >
                ...
                <button
                  ...
                  className={`p-1.5 rounded border border-[#CBD5E1] dark:border-[#1E293B] transition-colors cursor-pointer ${
                    autoScroll ? 'bg-amber-50 dark:bg-[#0B0B0C] text-amber-700 dark:text-[#FFC107]' : 'bg-white dark:bg-[#0B0B0C] text-[#64748B] hover:text-[#475569] dark:hover:text-[#94A3B8]'
                  }`}
                ...
                <button
                  onClick={() => setMessages([])}
                  className="p-1.5 rounded bg-white dark:bg-[#0B0B0C] border border-[#CBD5E1] dark:border-[#1E293B] hover:bg-slate-100 dark:hover:bg-[#1E293B] text-[#64748B] hover:text-rose-500 dark:hover:text-rose-400 transition-colors cursor-pointer"
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-[#F8FAFC] dark:bg-[#0B0B0C] border border-[#CBD5E1] dark:border-[#1E293B] rounded px-2.5 py-1.5 text-[11px] text-[#0F172A] dark:text-[#F8FAFC] focus:outline-none focus:border-amber-500 dark:focus:border-[#FFC107] font-mono min-w-0"
                />
                <button
                  onClick={() => validateAndSubscribe(topicInput)}
                  className="px-3 py-1.5 bg-amber-500 dark:bg-[#FFC107] hover:bg-amber-600 dark:hover:bg-[#FFB300] text-[#0B0B0C] font-bold rounded transition-colors font-mono text-xs cursor-pointer shrink-0"
                >
                  Subscribe
                </button>
              </div>
          ...
          <div
            ref={containerRef}
            className="flex-1 overflow-y-auto p-3 space-y-1.5 scrollbar-thin scrollbar-thumb-[#CBD5E1] dark:scrollbar-thumb-[#1E293B]"
          >
            {activeTopics.length === 0 ? (
              <div className="text-center py-16 text-[#64748B]">
                <Workflow className="w-10 h-10 mx-auto mb-3 text-[#64748B]" />
                <h3 className="text-[#475569] dark:text-[#94A3B8] font-semibold">Enter a Kafka topic (no wildcards).</h3>
                <p className="text-[10px] mt-1 text-[#64748B]">
                  e.g. <code className="text-amber-700 dark:text-[#FFC107]">uns.telemetry.raw</code>, <code className="text-amber-700 dark:text-[#FFC107]">uns.events.normalized</code>
                </p>
              </div>
            ) : messages.length === 0 ? (
              <div className="text-center py-16 text-[#64748B]">
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 rounded border text-xs cursor-pointer transition-all ${
                      isSelected
                        ? 'bg-slate-100 dark:bg-[#1E293B] border-amber-500 dark:border-[#FFC107] shadow-lg'
                        : 'bg-white dark:bg-[#0B0B0C] border-[#E2E8F0] dark:border-[#1E293B] hover:bg-slate-50 dark:hover:bg-[#111114] hover:border-[#CBD5E1] dark:hover:border-[#334155]'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span className="px-1.5 py-0.2 rounded bg-[#F1F5F9] dark:bg-[#111114] border border-[#E2E8F0] dark:border-[#1E293B] text-amber-700 dark:text-[#FFC107] text-[10px] font-bold">
                          {msg.topic}
                        </span>
                    ...
                    <div className="font-mono text-[10px] text-[#475569] dark:text-[#94A3B8] truncate bg-[#F8FAFC] dark:bg-[#050505] rounded px-1.5 py-0.5 border border-[#E2E8F0] dark:border-[#1E293B]">
        ...
        <div className={`lg:col-span-5 flex flex-col h-full bg-[#F8FAFC] dark:bg-[#0B0B0C] overflow-hidden p-4 space-y-4 ${
          mobileTab === 'stream' ? 'hidden lg:flex' : 'flex'
        }`}>
          <div className="pb-2 border-b border-[#E2E8F0] dark:border-[#1E293B] flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="font-bold text-[#0F172A] dark:text-[#F8FAFC] text-xs uppercase tracking-wider">Kafka Message Detail</span>
              {selectedMessage && (
                <span className="px-1.5 py-0.2 rounded bg-slate-200 dark:bg-[#1E293B] text-amber-700 dark:text-[#FFC107] text-[9px] font-mono">
          ...
          {selectedMessage ? (
            <div className="flex-1 overflow-y-auto space-y-3 scrollbar-thin scrollbar-thumb-[#CBD5E1] dark:scrollbar-thumb-[#1E293B]">
              {/* Metadata Card */}
              <div className="p-3 rounded bg-white dark:bg-[#111114] border border-[#E2E8F0] dark:border-[#1E293B] space-y-1.5 text-[10px]">
                <div className="flex justify-between">
                  <span className="text-[#64748B]">Topic:</span>
                  <span className="text-amber-700 dark:text-[#FFC107] font-semibold">{selectedMessage.topic}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#64748B]">Timestamp:</span>
                  <span className="text-[#0F172A] dark:text-[#F8FAFC]">{selectedMessage.timestamp}</span>
                </div>
                {selectedMessage.key && (
                  <div className="flex justify-between">
              ...
              {/* JSON Payload Viewer */}
              <div className="space-y-1.5">
                <span className="text-[10px] text-[#475569] dark:text-[#94A3B8] font-bold uppercase tracking-wider">Payload:</span>
                <div className="bg-white dark:bg-[#111114] border border-[#E2E8F0] dark:border-[#1E293B] rounded-lg p-3 overflow-x-auto">
              ...
              {selectedMessage.headers && Object.keys(selectedMessage.headers).length > 0 && (
                <div className="space-y-1.5">
                  <span className="text-[10px] text-[#475569] dark:text-[#94A3B8] font-bold uppercase tracking-wider">Kafka Headers:</span>
                  <div className="bg-white dark:bg-[#111114] border border-[#E2E8F0] dark:border-[#1E293B] rounded-lg p-3 text-[10px]">
                    <table className="w-full">
                      <tbody>
                        {Object.entries(selectedMessage.headers).map(([k, v]) => (
                          <tr key={k} className="border-b border-[#E2E8F0]/50 dark:border-[#1E293B]/50 last:border-0">
                            <td className="text-[#64748B] py-1 font-mono">{k}</td>
                            <td className="text-[#0F172A] dark:text-[#F8FAFC] py-1 font-mono text-right">{v}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-[#64748B]">
              <Workflow className="w-10 h-10 mb-2 opacity-40 text-amber-500 dark:text-[#FFC107]" />
              <p className="text-xs text-[#475569] dark:text-[#94A3B8]">Select a Kafka message from the stream to inspect payload &amp; headers.</p>
            </div>
          )}
        </div>
      </div>
